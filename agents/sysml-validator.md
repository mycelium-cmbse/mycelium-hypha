...
- `knowledge/sysml2/` – the combined KerML + SysML v2 metamodel: `index.json` (or `index.md`) to
...
3. Ground every ruling, delegating the lookup to the right knowledge:
   - **metamodel structure** – resolve the metaclass via `knowledge/sysml2/index.json`, then read its
...
- **Reference** – the `knowledge/sysml2/elements/<Metaclass>.md` or `knowledge/spec/…` path behind the
