---
name: USCustomaryUnits
kind: model-library-file
language: SysML
source: sysml.library/Domain Libraries/Quantities and Units/USCustomaryUnits.sysml
declares: [acre (based on US survey foot), acre foot (based on US survey foot), circular mil, clo, cord (128 ft^3), cup (US), fathom (based on US survey foot), foot (US survey), footcandle, footlambert, foot of water (39.2 °F), foot poundal, horsepower (boiler), horsepower (electric), horsepower (water), hundredweight (long, 112 lb), hundredweight (short, 100 lb), inch of mercury (32 °F), inch of mercury (60 °F), inch of water (39.2 °F), inch of water (60 °F), microinch, mile (based on US survey foot), ohm circular-mil per foot, ounce (US fluid), pica (printer′s), point (printer′s), pound-force per square inch (psi), quad (10^15 Btu_IT), square mile (based on US survey foot), tablespoon, teaspoon, therm (EC), therm (US), ton-force (2000 lbf), ton, long (2240 lb), ton, long, per cubic yard, ton of refrigeration (12 000 Btu_IT/h), ton, register, ton, short (2000 lb), ton, short, per cubic yard, ton, short, per hour, unit pole, temperatureWaterAtFreezingPointInF, fahrenheitToCelsiusScaleMapping, zeroDegreeFahrenheitInKelvin, zeroDegreeFahrenheitToKelvinShift]
license: EPL-2.0
---

# USCustomaryUnits

Verbatim SysML standard-library source from `sysml.library/Domain Libraries/Quantities and Units/USCustomaryUnits.sysml` (EPL-2.0; see [NOTICE](../../../NOTICE)).
...
- `acre (based on US survey foot)` — attribute
- `acre foot (based on US survey foot)` — attribute
- `circular mil` — attribute
- `clo` — attribute
- `cord (128 ft^3)` — attribute
- `cup (US)` — attribute
- `fathom (based on US survey foot)` — attribute
- `foot (US survey)` — attribute
- `footcandle` — attribute
- `footlambert` — attribute
- `foot of water (39.2 °F)` — attribute
- `foot poundal` — attribute
- `horsepower (boiler)` — attribute
- `horsepower (electric)` — attribute
- `horsepower (water)` — attribute
- `hundredweight (long, 112 lb)` — attribute
- `hundredweight (short, 100 lb)` — attribute
- `inch of mercury (32 °F)` — attribute
- `inch of mercury (60 °F)` — attribute
- `inch of water (39.2 °F)` — attribute
- `inch of water (60 °F)` — attribute
- `microinch` — attribute
- `mile (based on US survey foot)` — attribute
- `ohm circular-mil per foot` — attribute
- `ounce (US fluid)` — attribute
- `pica (printer′s)` — attribute
- `point (printer′s)` — attribute
- `pound-force per square inch (psi)` — attribute
- `quad (10^15 Btu_IT)` — attribute
- `square mile (based on US survey foot)` — attribute
- `tablespoon` — attribute
- `teaspoon` — attribute
- `therm (EC)` — attribute
- `therm (US)` — attribute
- `ton-force (2000 lbf)` — attribute
- `ton, long (2240 lb)` — attribute
- `ton, long, per cubic yard` — attribute
- `ton of refrigeration (12 000 Btu_IT/h)` — attribute
- `ton, register` — attribute
- `ton, short (2000 lb)` — attribute
- `ton, short, per cubic yard` — attribute
- `ton, short, per hour` — attribute
- `unit pole` — attribute
- `temperatureWaterAtFreezingPointInF` — attribute
- `fahrenheitToCelsiusScaleMapping` — attribute
- `zeroDegreeFahrenheitInKelvin` — attribute
- `zeroDegreeFahrenheitToKelvinShift` — attribute
