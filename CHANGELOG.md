...
- `knowledge/cross-references.json` (+ schema): edges from every metamodel element to the
  ...
  layer (`fixes #68`).
- Provenance tiers (`NORMATIVE` / `MODEL` / `DERIVED` / `PILOT`) documented in the skills and the
  README, carried in the data itself via the `provenanceTiers` block, with a curated
  `pilotDivergences` array reserved for pilot-implementation divergences.
